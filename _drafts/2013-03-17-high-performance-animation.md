...
即使使用rAF，还是会有诸多的不便。我们还有一个选择是使用css动画。虽然我们之前说浏览器中UI线程与js线程是互斥，但这一点对css动画不成立。

在这里不聊css动画的用法。我想聊聊css动画，又不仅仅是动画，而是如何利用css提升浏览器性能。

首先我们看看淘宝首页的焦点图：

![taobao](./images/taobao_3d.png)

我想提出的问题是为什么明明可以使用translate 2d去实现的动画，它要用3d去实现呢？

我不是淘宝的员工，但我猜测的这么做的原因是为了hack。简单来说如果你给一个元素添加上了`-webkit-transform: translateZ(0);`或者` -webkit-transform: translate3d(0,0,0);`属性，那么你就告诉浏览器用GPU来选来该层(虽然我很确定这么做会在Chrome中启用了硬件加速，但在其他平台比如Firefox、Safari也大致适用)。提升了速度和性能。不信你可以给一个带有焦点图的网页试一试

// todo

但这样的说法其实并不准确，至少在现在的Chrome版本中这算不上一个hack(默认渲染所有的网页时都会使用GPU渲染)。再了解这么的做的原理之前，你必须先了解一个层(Layer)的概念。
