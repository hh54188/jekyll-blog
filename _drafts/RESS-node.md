...
```
从图中继续可以看出，很明显内容区域使用流式布局可以同时为移动和桌面公用，但导航区域却大相径庭。于是我们想复用内容区域部分，而将导航部分使用不同的模板渲染，区分开。

此时此刻忘掉media query吧，用户永远是无辜的，为什么他在某时某刻某台设备上访问的页面却有兼容多台设备的代码呢。
...
为了兼容不同平台支持的视频格式，视频代码也不得不准备一式三份。如果我们能够知道用户设备支持视频格式的情况，那么同样在后台渲染页面时就可以避免累赘的代码传递给用户：
```
<video>
...
例如我们会使用到CSS3动画来增添一些特效。注意这里是增添，也就意味着我们是在做加法。于是我们会准备两份样式，一份名为`content.css`，也就是通用的基础样式，关于配色关于布局，这都不重要——同时我们也会准备另一份样式`animate.css`，仅包含我们利用CSS3写的动画特效。

通常情况下我们会把这两份样式同时返回给用户，但很明显并非所有用户的浏览器都支持CSS3动画，即使不支持也是无害的，但代价就是用户可能白白多承受几KB的流量。要知道CSS3的动画代码同样是一式三份（甚至四份）极其臃肿的：
```
.classname {
...
特性检测就登场了。我们通过检测用户的设备是否支持CSS3动画来决定返回给用户的样式中是否包含动画样式：
...
在我们通过检测动画特性返回特定样式一节中，不知道你有没有考虑过这样一个问题：假设一段时间内访问我们页面的用户设备都是支持的CSS3动画的，那么每一位用户访问时我们都必须重新生成一遍相同的样式文件。为什么不把上一次生成的结果暂存下来等到这次返回？于是我们需要缓存机制，将上一次打包结果缓存起来，以便下一次直接使用。
...
return result;
```
代码中的逻辑基本上和本节开头所述是一致的。唯一有一个使用技巧的地方是，在判断缓存是否存在的`if`语句中，我使用的是赋值表达式`result = cache.getCache(supportedKey)`。这么做的好处有
1. 我将从缓存中读取道德缓存结果返回给`result`
2. 同时，赋值表达式也是有返回结果的，在这里赋值表达式返回的是result的值，这样一来我就能判断缓存结果是否存在`!result`

所以语句`if (!(result = cache.getCache(supportedKey)))`同时做了两件事，从cache中读取缓存，同时判断缓存是否存在。
